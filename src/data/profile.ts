export const profile = {
  name: 'Rasika Edirisinghe',
  role: 'MSCA Doctoral Researcher',
  institution: 'University College Cork',
  location: 'Cork, Ireland',
  programme: 'CASCADE Horizon Europe Marie Skłodowska-Curie Doctoral Network',
  researchStatement: 'Textual relations under transformation',
  conceptualStatement: 'Representation · Retrieval · Judgement · Interpretation',
  question: 'How can relationships between texts be recovered as their wording changes?',
  description: 'Doctoral researcher at University College Cork working on the computational representation, retrieval, and visual analysis of textual relations under transformation.',
  links: [
    { label: 'UCC email', href: 'mailto:rasika.edirisinghe@ucc.ie', external: false },
    { label: 'ORCID', href: 'https://orcid.org/0009-0008-4673-0259', external: true },
    { label: 'Scholar', href: 'https://scholar.google.com/citations?user=o_7ZjgMAAAAJ&hl=en', external: true },
    { label: 'GitHub', href: 'https://github.com/mrHendrixSL/', external: true },
    { label: 'LinkedIn', href: 'https://www.linkedin.com/in/rasikaedirisinghe/', external: true },
    { label: 'CV', href: '/assets/Rasika_Edirisinghe-CV.pdf', external: true },
    { label: 'Notes', href: '/notes/', external: false }
  ]
} as const;
